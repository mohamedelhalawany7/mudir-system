...
import re
import base64
from filelock import FileLock # 🟢 مكتبة حماية الملفات للاستقرار المتزامن

# ============================================================
# ░█▀▀░█░░░▀█▀░▀█▀░█▀▀░░░█▀█░█▀▀░░░█░█░▀▀   MUDIR OS v49.0 (STABLE CONCURRENCY)
# ============================================================
st.set_page_config(
...
2. تابعه واسأله عن الوقت خده، لو اتأخر أو مفيش نتيجة، كن حازم ووبخه بشياكة كمدير. لو شاطر شجعه بكلمة (عاش يا بطل).
3. قيم التقارير التي يقدمها الموظف هنا في الشات واعتمد عليها لتقييمه.
4. تجنب استخدام الرموز التعبيرية تماماً عشان تحافظ على هيبتك كمدير. استخدم التنسيق الواضح بالماركداون (عناوين، نقاط، أرقام)."""
...
def save_config(cfg_dict):
    """حفظ الإعدادات بأمان تام لمنع انهيار JSON"""
    if 'workspace_id' in st.session_state:
        ws_file = get_workspace_file()
        lock = FileLock(f"{ws_file}.lock")
        with lock:
...
def atomic_save_chat_and_log(user, msg_dict=None, delete_idx=None):
    """دالة آمنة لحفظ المحادثات متزامنة (Concurrency Proof)"""
    ...
    with lock:
        try:
            ...
            if 'ALL_CHATS' not in disk_cfg: disk_cfg['ALL_CHATS'] = {}
            if 'AUDIT_LOG' not in disk_cfg: disk_cfg['AUDIT_LOG'] = {}

            if user not in disk_cfg['ALL_CHATS']: disk_cfg['ALL_CHATS'][user] = []
            if user not in disk_cfg['AUDIT_LOG']: disk_cfg['AUDIT_LOG'][user] = []

            # Perform actions
            if delete_idx is not None:
                if 0 <= delete_idx < len(disk_cfg['ALL_CHATS'][user]):
                    disk_cfg['ALL_CHATS'][user].pop(delete_idx)
            elif msg_dict is not None:
                disk_cfg['ALL_CHATS'][user].append(msg_dict)
                entry = msg_dict.copy()
                entry['timestamp'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                disk_cfg['AUDIT_LOG'][user].append(entry)
            ...
            # Update memory
            st.session_state.app_config = disk_cfg
            st.session_state.all_chats = disk_cfg['ALL_CHATS']
...
def load_licenses():
    if os.path.exists(LICENSES_FILE):
        lock = FileLock(f"{LICENSES_FILE}.lock")
        with lock:
            try:
        ...
        if "المدير العام" in selected_user:
            if pin == st.session_state.app_config.get('MANAGER_PIN', '0000'):
                ...
                if selected_user not in st.session_state.all_chats:
                    ...
                    atomic_save_chat_and_log(selected_user, initial_msg)
                st.rerun()
            else:
            ...
            if pin == expected_pin:
                ...
                if selected_user not in st.session_state.all_chats:
                    ...
                    initial_msg = {"role": "assistant", "content": f"أهلاً بيك يا {emp_name_only}. أنا مديرك. مفيش وقت نضيعه، وريني التقارير اللي معاك عشان أديك تكليفاتك."}
                    atomic_save_chat_and_log(selected_user, initial_msg)
                st.rerun()
            else:
...
# ────────────────────────────────────────────────────────────
@st.dialog("تقرير تقييم الأداء التفصيلي (النظام المستقل)", width="large")
def show_employee_report_dialog(emp_full_name, start_date, end_date):
    ...
    # 2. Get Evals & Chats (No Odoo Dependency)
    ...
    for al in audit_log:
        try:
            ...
            if start_date <= al_date <= end_date:
                ...
                active_dates.add(al_date)
        except: pass

    total_interactions = len(activities)
    evals_count = len(filtered_evals)
    active_days = len(active_dates)

    # Generate Smart Report
    with st.spinner("جاري تحليل تقارير الموظف والمحادثات لإنشاء تقييم شامل..."):
        ...
        report_prompt = f"""
        أنت خبير تقييم أداء (HR Executive). قم بكتابة تقرير أداء ذكي وملخص لموظف بناءً على المهام وتقارير المحادثات معه فقط:
        - اسم الموظف: {emp_short}
        - الوظيفة: {emp_role}
        - إجمالي التفاعلات في النظام (التقارير المرفوعة): {total_interactions}
        - عدد أيام العمل النشطة: {active_days} يوم
        - عدد التقييمات المسجلة من المدير: {evals_count}
        - الأهداف المطلوبة من الموظف (KPIs): {kpis}

        سجل التقييمات السابقة (مستخرجة من تعليقات المدير):
        {evals_str if evals_str else 'لا يوجد'}

        مقتطفات من تفاعلات الموظف (التقارير والنقاشات):
        ...
        قسم التقرير إلى:
        1. الخلاصة التنفيذية.
        2. تقييم أداء المهام وجودة التقارير.
    ...
    <body>
        <div class="report-container">
            <div class="header">
                <h1>تقرير الأداء والتفاعل الذكي</h1>
                <div style="color: #64748b; font-size: 15px;">يعتمد على التواصل والتقارير المرفوعة فقط</div>
            </div>
            ...
            <table class="metrics-table">
                <tr>
                    <td>
                        <div class="m-title">إجمالي التفاعلات</div>
                        <div class="m-val" style="color: #059669;">{total_interactions}</div>
                    </td>
                    <td>
                        <div class="m-title">أيام نشطة</div>
                        <div class="m-val" style="color: #2563eb;">{active_days}</div>
                    </td>
                    <td>
                        <div class="m-title">تقييمات مسجلة</div>
                        <div class="m-val" style="color: #d97706;">{evals_count}</div>
                    </td>
                </tr>
            </table>
            ...
            <div class="footer">
                تم استخراج هذا التقرير آلياً بواسطة محرك الذكاء الاصطناعي المستقل - MUDIR OS<br>
    ...
    <div style="background-color: #111b21; border-radius: 16px; padding: 30px; border: 1px solid #202c33; box-shadow: 0 8px 32px rgba(0,0,0,0.4); direction: rtl; font-family: 'Cairo', sans-serif;">
        <div style="text-align: center; border-bottom: 2px solid #202c33; padding-bottom: 20px; margin-bottom: 30px;">
            <h2 style="color: #00a884; font-weight: 800; font-size: 2rem; margin: 0 0 10px 0;">التقرير الإداري الشامل (مستقل)</h2>
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 30px;">
            <div style="flex: 1; background: #202c33; padding: 20px; border-radius: 12px; text-align: center;">
                <div style="color: #8696a0; font-size: 0.9rem; font-weight: 600; margin-bottom: 5px;">إجمالي التفاعلات</div>
                <div style="color: #00a884; font-size: 1.8rem; font-weight: 800;">{total_interactions}</div>
            </div>
            <div style="flex: 1; background: #202c33; padding: 20px; border-radius: 12px; text-align: center;">
                <div style="color: #8696a0; font-size: 0.9rem; font-weight: 600; margin-bottom: 5px;">أيام عمل نشطة</div>
                <div style="color: #00f2ff; font-size: 1.8rem; font-weight: 800;">{active_days}</div>
            </div>
            <div style="flex: 1; background: #202c33; padding: 20px; border-radius: 12px; text-align: center;">
                <div style="color: #8696a0; font-size: 0.9rem; font-weight: 600; margin-bottom: 5px;">تقييمات مسجلة</div>
                <div style="color: #ffd700; font-size: 1.8rem; font-weight: 800;">{evals_count}</div>
            </div>
        </div>
...
def render_ai():
    ...
    team_context_str = "\n".join(team_context_lines) if team_context_lines else "لا توجد تفاعلات لزملاء آخرين حالياً."
    ...
    if "المدير العام" in curr_user:
                ...
                if sel_emp:
                    ...
                    with c_arc1:
                        if st.button(f"🗑️ مسح واجهة الشات لـ {sel_emp.split(' - ')[0]}", use_container_width=True):
                            ...
                            atomic_save_chat_and_log(sel_emp, {"role": "assistant", "content": "تم مسح الأرشيف بواسطة الإدارة."})
                            st.rerun()
                    with c_arc2:
                        if st.button(f"🔄 استعادة المحادثة بالكامل من السجل السري", use_container_width=True, type="primary"):
                            ...
                            if audit_history:
                                ...
                                with lock:
                                    try:
                                        with open(ws_file, 'r', encoding='utf-8') as f: disk_cfg = json.load(f)
                                        disk_cfg['ALL_CHATS'][sel_emp] = st.session_state.all_chats[sel_emp]
                                        with open(ws_file, 'w', encoding='utf-8') as f: json.dump(disk_cfg, f, ensure_ascii=False, indent=4)
                                    except: pass
                    ...
                    for idx, m in enumerate(chat_to_view):
                        with st.chat_message(m["role"]):
                            ...
                            if st.button("🗑️", key=f"gm_dl_{sel_emp}_{idx}", help="حذف الرسالة"):
                                atomic_save_chat_and_log(sel_emp, delete_idx=idx)
                                st.rerun()
                            st.markdown('</div>', unsafe_allow_html=True)
            else:
                ...
                for idx, msg in enumerate(st.session_state.all_chats.get(curr_user, [])):
                    with st.chat_message(msg["role"]):
                        ...
                        if st.button("🗑️", key=f"dl_{curr_user}_{idx}", help="حذف الرسالة"):
                            atomic_save_chat_and_log(curr_user, delete_idx=idx)
                            st.rerun()
                        st.markdown('</div>', unsafe_allow_html=True)
        ...
        user_input = st.chat_input("ارفع تقريرك للمدير هنا...")
    ...
    if user_input:
        ...
        atomic_save_chat_and_log(curr_user, user_msg)
        ...
        with chat_area:
            ...
            with st.spinner("المدير يقرأ ويكتب الآن..."):
...
الموظف الذي يحدثك الآن: {curr_user}.
"""
                if curr_user != "المدير العام":
                    live_context += f"\n=== ملف الموظف الحالي ({curr_user}) ===\n"
                    live_context += f"المهام والأهداف المطلوبة من هذا الموظف (KPIs):\n{job_desc}\n"
                    live_context += "بصفتك المدير، قم بتوجيه الموظف ومحاسبته وإعطائه مهام وتقييم التقارير المكتوبة التي يقدمها لك بدقة وبدون أي مجاملة.\n"

                live_context += """
**مهم جداً جداً للتقييم**: في نهاية أي رسالة بتكتبها، لازم تقيم الموظف (إذا لم يكن هو المدير العام) من 10 وتكتب تعليق سري عن أدائه في الشات. التقييم ده لازم يكون في آخر سطر بالظبط بالشكل ده:
                ...
                try:
                    ...
                    if eval_match:
                        ...
                        if "المدير العام" not in curr_user:
                            if 'EVALUATIONS' not in CFG: CFG['EVALUATIONS'] = {}
                            if 'EVAL_HISTORY' not in CFG: CFG['EVAL_HISTORY'] = {}
                            if curr_user not in CFG['EVAL_HISTORY']: CFG['EVAL_HISTORY'][curr_user] = []
                            
                            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                            CFG['EVALUATIONS'][curr_user] = {'eval': eval_data, 'date': now_str}
                            CFG['EVAL_HISTORY'][curr_user].append({'eval': eval_data, 'date': now_str})
                            save_config(CFG)
                    
                    response_text = re.sub(r'\$\$EVAL:.*', '', response_text, flags=re.DOTALL).strip()

                    ai_final_msg = {"role": "assistant", "content": response_text}
                    atomic_save_chat_and_log(curr_user, ai_final_msg)
                    st.rerun()
                except Exception as e:
                    atomic_save_chat_and_log(curr_user, delete_idx=-1)
                    err_msg = str(e).lower()
