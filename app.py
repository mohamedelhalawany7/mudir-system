...
# =====================================================================
DEFAULT_SYSTEM_PROMPT = """أنت المحرك الذكي لنظام الإدارة.
[القاعدة الذهبية والمطلقة]: 
يجب عليك التعامل مع الموظف الحالي الذي يتحدث معك بناءً على "التعليمات الخاصة بالموظف" المرفقة في السياق أدناه حصراً. 
شخصيتك، طريقة ردك، والمهام التي تطلبها يجب أن تتوافق 100% مع التعليمات الخاصة بهذا الموظف تحديداً.

[بروتوكول الأوامر السرية - هام جداً للإدارة]:
أنت ترد بنص عادي (Markdown). لكن لتنفيذ إجراء في النظام، أضف العلامات التالية في **نهاية** رسالتك تماماً:
- لإسناد مهمة: [TASK: وصف المهمة]
- لإنهاء مهمة: [CLOSE_TASK: اسم المهمة]
- لتقييم الموظف: [EVAL: التقييم والسبب]
- لحفظ معلومة بذاكرتك: [MEMO: المعلومة]
- لإنشاء عرض سعر: [ACTION: CREATE_SO|العميل:اسم العميل|القيمة:1000]

استخدم الجداول (Markdown Tables) لعرض البيانات، وتجنب استخدام Emojis نهائياً ما لم يطلب منك العكس في تعليمات الموظف.
"""
...
.chat-bubble p { margin: 0 !important; padding: 0 !important; color: #e9edef !important; font-size: 14.2px !important; line-height: 1.6 !important; display: block !important;}
...
def build_ai_context(curr_user, CFG, df_s, df_p):
    ...
    # تنسيق مضغوط للسياق مع إعطاء الأولوية القصوى لتعليمات الموظف
    live_context = (
