...
def load_config():
    defaults = {
        ...
        'EMPLOYEES': [], 
        'EVALUATIONS': {},
        'EVAL_HISTORY': {}, # سجل التقييمات التاريخي
        'ALL_CHATS': {},
...
def show_employee_report_dialog(emp_full_name, start_date, end_date):
    ...
    with st.spinner("جاري تحليل محادثات وتقارير الموظف استخباراتياً..."):
        ...
        المطلوب:
        اكتب تقرير إداري مكثف ومنظم بصيغة HTML (بدون استخدام Markdown نهائياً)، ليكون مناسباً للطباعة الفورية.
        ...
        3. نقاط القوة، والمجالات التي تحتاج لتحسين.
        ...
        try:
            ...
            smart_report_html = f"""
            <div style="background-color: #fff1f2; border-right: 4px solid #e11d48; padding: 20px; border-radius: 8px; margin-top: 15px;">
                <h3 style="color: #e11d48; margin-top: 0; display: flex; align-items: center; gap: 8px;">
                    ⚠️ تعذر إنشاء التقرير الذكي
                </h3>
                <p style="color: #334155; font-size: 15px; margin-bottom: 10px;">
                    نعتذر، لم يتمكن النظام من الاتصال بمحرك الذكاء الاصطناعي لتحليل البيانات وتوليد التقرير في الوقت الحالي. قد يكون ذلك بسبب ضغط على الخوادم المركزية أو خطأ في إعدادات الربط.
                </p>
                <div style="background: #fee2e2; color: #991b1b; padding: 10px; border-radius: 6px; font-size: 13px; font-family: monospace; text-align: left; direction: ltr;">
                    Error Details: {str(e)}
                </div>
            </div>
            """
                    ...
                    if eval_match:
                        ...
                        if "المدير العام" not in curr_user:
                            if 'EVALUATIONS' not in CFG:
                                CFG['EVALUATIONS'] = {}
                            if 'EVAL_HISTORY' not in CFG:
                                CFG['EVAL_HISTORY'] = {}
                            if curr_user not in CFG['EVAL_HISTORY']:
                                CFG['EVAL_HISTORY'][curr_user] = []
                            
                            now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                            # حفظ الأخير للظهور السريع
                            CFG['EVALUATIONS'][curr_user] = {'eval': eval_data, 'date': now_str}
                            # حفظ في سجل التقييمات الشامل للتقارير
