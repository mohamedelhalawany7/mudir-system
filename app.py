...
def render_ai():
    ...
    live_cfg = safe_json_read(ws_file, {})
    if 'ALL_CHATS' in live_cfg:
        st.session_state.all_chats = live_cfg['ALL_CHATS']
...
def render_settings():
    ...
    if submit_emp:
        if len(current_emps) >= max_devices:
            ...
            ws_file = get_workspace_file()
            live_cfg = safe_json_read(ws_file, {})
            live_cfg['EMPLOYEES'] = current_emps
            safe_json_write(ws_file, live_cfg)
...
def change_workspace_pin_dialog(ws_id):
    ...
    if st.button("حفظ التغيير", type="primary", use_container_width=True):
        ...
        safe_json_write(ws_file, ws_cfg)
...
def render_super_admin():
    ...
    with sv1:
        ...
        for ws in licenses['workspaces'].keys():
            try:
                ws_file = f"mudir_workspace_{ws}.json"
                if os.path.exists(ws_file):
                    full_platform_backup["workspaces_db"][ws] = safe_json_read(ws_file, {})
            except: pass
                ...
                try:
                    ...
                    if "workspaces_db" in restored_mega:
                        for ws, ws_data in restored_mega["workspaces_db"].items():
                            safe_json_write(f"mudir_workspace_{ws}.json", ws_data)
    ...
    if add_btn:
        ...
        if not safe_id:
            ...
            safe_json_write(ws_file, initial_config)
